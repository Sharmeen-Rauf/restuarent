...
export default function MenuSection({ activeCategory, onCategoryChange }: MenuSectionProps) {
  const { addToCart, cart, setIsCartOpen, setSelectedItem, setIsProductModalOpen } = useCart();
  ...
  const handleItemAddClick = (e: React.MouseEvent, item: MenuItem) => {
    e.stopPropagation();
    const cartItem = cart.find(i => i.id === item.id);
    if (cartItem) {
      // Item already in cart, open detail modal to update
      setSelectedProduct(item);
      setIsProductModalOpen(true);
    } else {
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {quantity > 0 ? (
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-semibold text-black w-8 text-center">{quantity}</span>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  addToCart(item);
